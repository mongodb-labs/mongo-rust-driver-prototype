...
#[test]
fn test_optional_user_password() {
...
#[test]
fn test_default_port_on_single_host() {
...
#[test]
fn test_default_port_on_replica_set() {
...
fn test_query_separators() {
    for delim in vec!(";", "&") {
        ...
        assert_eq!("true", options.get_str("slaveOk").unwrap());
        assert_eq!("myreplset", options.get_str("replicaSet").unwrap());
        assert_eq!("1", options.get_str("x").unwrap());
    }
}

#[test]
fn test_read_pref_tags() {
    let pref_set = vec!(
        vec!("dc:ny"),
        vec!("dc:ny,rack:1"),
...
#[test]
fn test_unix_domain_socket_auth() {
...
#[test]
fn test_unix_domain_socket_replica_set() {
...
fn test_full() {
    ...
    let options = connstr.options.unwrap();
    assert_eq!("myreplset", options.get_str("replicaSet").unwrap());
    assert_eq!("true", options.get_str("journal").unwrap());
    assert_eq!("50", options.get_str("wtimeoutMS").unwrap());
}
